...
options(warn = -1)
library("knitr")
opts_knit$set(root.dir = "~/Dropbox/Boston_Experiment/Physiology/phys_R_code/16Jul16 Update/Physiology_for_Chapter_3_files")
```
...
df <- read_csv("/Users/colleen/Dropbox/Boston_Experiment/Physiology/phys_R_code/16Jul16 Update/Physiology_for_Chapter_3_files/phys_all_13Aug19.csv")
...
```{r, echo=FALSE, fig.height = 7, fig.width = 9, fig.align = "center", results = 'hide'}
df2 <- df[,c(1:16,38)]
...
comp.sum <- summarySE(data = df.comp, measurevar = "value", groupvars = c("param", "species", "treat", "T0_T90"), na.rm = TRUE)

comp.fig <-ggplot(comp.sum, aes(colour=treat, shape = treat))+
  theme(legend.position="bottom", panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), axis.line=element_line(color="black"), legend.key=element_blank(), strip.background = element_blank(), strip.placement = "outside", panel.border = element_rect(colour = "black", fill=NA, size=1))+
  xlab("time point")+
  ylab("")+
  geom_errorbar(aes(x = T0_T90, ymin=value-se, ymax=value+se, width=0.5, colour=treat), position=dodge2)+
<<<<<<< HEAD
  geom_point(aes(T0_T90, value), position = dodge2, size=3)+
  ...
  facet_wrap(param~species, scales = "free", strip.position = "left", labeller = as_labeller( c(chla = "Chlorophyll \n (ug/cm2)", den = "Symbiont Density \n (10^6 cells cm2)", pro = "Total Protein \n (mg/cm2)", carb = "Carbohydrate \n (mg/cm2)", S = "SSID", P = "PSTR", A = "PAST", T = "UTEN")))
comp.fig
=======
  geom_point(aes(T0_T90, value), position = dodge2, size=2)+
  scale_colour_manual("Treatment", labels=c("control", "280_28", "280_31", "400_28", "400_31", "700_28", "700_31", "2800_28", "2800_31"), values = c("#009E73", "#0072B2", "#D55E00", "#0072B2", "#D55E00", "#0072B2", "#D55E00", "#0072B2", "#D55E00"))+
  scale_shape_manual("Treatment", labels=c("control", "280_28", "280_31", "400_28", "400_31", "700_28", "700_31", "2800_28", "2800_31"), values = c(8, 18, 18, 15, 15, 16, 16, 17, 17))+
  facet_wrap(param~species, scales = "free", strip.position = "left", labeller = as_labeller( c(chla = "Chlorophyll \n (ug/cm2)", den = "Symbiont Density \n (10^6 cells cm2)", pro = "Total Protein \n (mg/cm2)", carb = "Carbohydrate \n (mg/cm2)", S = "", P = "", A = "", T = "")))
...
corals <- cowplot::plot_grid(s, p, a, u, labels = "AUTO", ncol = 4)

full.T0.fig <- plot_grid(corals, comp.fig, ncol = 1, align = "v", axis = 'l', rel_heights = c(1,4))
full.T0.fig


>>>>>>> 2f7207804f289f2afc4595a2088bec2bc7a0240a


#p <- ggplot2::ggplot(iris, aes(x = Sepal.Length, fill = Species)) +  ggplot2::geom_density(alpha = 0.7)
#p2 <- cowplot::ggdraw() + cowplot::draw_image("", scale = 0.9)
#cowplot::plot_grid(p, p2, labels = "AUTO")
#
#https://github.com/wilkelab/cowplot/issues/84
