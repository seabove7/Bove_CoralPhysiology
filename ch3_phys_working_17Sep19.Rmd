...
df.comp$species <- revalue(x = df.comp$species, c("S" = "SSID", "P" = "PSTR", "A" = "PAST", "T" = "UTEN"))
...
T90.df.pro.a <- read.csv("Protein_model_T90_2019-10-07.csv", head=T)
...
T90.df.carb.a <- read.csv("carbtein_model_T90_2019-10-07.csv", head=T)
...
T90.df.den.a <- read.csv("dentein_model_T90_2019-10-07.csv", head=T)
...
T90.df.chla.a <- read.csv("chlatein_model_T90_2019-10-07.csv", head=T)
...
pro.den.mod.s <- lm(en ~ pro * ftemp * fpco2, data = phys.s) # ftemp and fpco2
...
### Figure 8: *Porites astreoides*
*Models were fit by overall correlation (black), temperature (blue/red), or pCO2 (pink/green/orange/darkred) depending on best linear model fit*
  
```{r PAST comp model, echo=FALSE}
...
```
```{r PAST comp figures, echo=FALSE, fig.height=8, fig.width=10}
...
**Figure 8.** Text goes here


### Figure 9: *Undaria tenuifolia*
*Models were fit by overall correlation (black), temperature (blue/red), or pCO2 (pink/green/orange/darkred) depending on best linear model fit*
  
```{r UTEN comp model, echo=FALSE}
...
```
```{r UTEN comp figures, echo=FALSE, fig.height=4, fig.width=9}
