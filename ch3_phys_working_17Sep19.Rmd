...
```{r symb phys model, echo=FALSE, fig.height = 2, fig.width = 10, fig.align = "center", results = 'hide', message=FALSE}
...
T90.df.symb.a <- read.csv("symb_model_T90_2019-10-21.csv", head=T)
...
symb.model.fig <- ggplot(T90.df.symb.a)+
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), strip.text.x = element_blank(), panel.background=element_blank(), axis.line=element_line(color="black"), legend.key=element_blank(), strip.background = element_blank(), legend.position="none", panel.border = element_rect(colour = "black", fill=NA, size=1))+
  scale_x_discrete(limits=c(2, 4, 6, 8, 10))+
  geom_point(data=df.comp.symb, aes(x=treat, y=(value*10000000), group = species), alpha = 0.3, size = 2, shape = 8, colour = "#009E73")+
...
```{r figure 3 plot, echo=FALSE, fig.height = 5, fig.width = 10, fig.align = "center", results = 'hide'}
...
```
**Figure 3.** Modelled 95% confidence interval of *(a) S. siderea*, *(b) P. strigosa*, *(c) P. astreoides*, and *(d) U. tenuifolia* total host (top) or symbiont (bottom) physiology with individual coral fragment physiology denoted by points. Blue denotes 28C and red denotes 31C, with *p*CO2 treatment along the x axis.
<br/>
<br/>
