...
dlab<-expression(paste("Symbiont density \n (10"^6,"cells cm"^-2,")"))
plab<-expression(paste("Total protein \n (mg cm"^-2,")"))
calab<-expression(paste("Carbohydrate \n (mg cm"^-2,")"))
clab<-expression(paste("Chlorophyll a \n ("*mu,"g cm"^-2,")"))
rlab<-expression(paste("Calcification rate \n (mg cm"^-2~"day"^-1,")"))
date <- Sys.Date() 
```
...
comp.fig
=======
>>>>>>> 14853594f80c2e3a3218ded1c5c998fe5816dabf
...
#boots <- apply(out,2,function(x) predict(lmer(x ~ species * (fpco2 + ftemp) + (1 | colony), REML=F, #data = T90.df.pro),re.form=NA))
#
...
#boots <- apply(out,2,function(x) predict(lmer(x ~ species + fpco2 + ftemp + (1 | colony), REML=F, #data = T90.df.carb),re.form=NA)) 
#
...
#boots <- apply(out,2,function(x) predict(lmer(x ~ species + fpco2 + ftemp + (1 | colony), REML=F, #data = T90.df.den),re.form=NA))
#
...
#boots <- apply(out,2,function(x) predict(lmer(x ~ species * fpco2 * ftemp + (1 | colony), REML=F, #data = T90.df.chla),re.form=NA)) 
#
...
#boots <- apply(out,2,function(x) predict(lmer(x ~ species * (fpco2 + ftemp) + (1 | colony), REML=F, #data = T90.df.sum),re.form=NA))
#
...
# Model estimate 95% confidence interval for physiologies
```{r, echo=FALSE, fig.height = 20, fig.width = 10, fig.align = "center", results = 'hide'}
# full T90 plot with coral images
full.T90.model.fig <- plot_grid(corals, pro.model.fig, carb.model.fig, den.model.fig, chla.model.fig, sum.model.fig, ncol = 1, rel_heights = c(3,3,3,3,3,3))
