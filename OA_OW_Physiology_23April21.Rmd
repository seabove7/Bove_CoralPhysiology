...
s_sub_symb_pca <- autoplot(s_sub_pca, data = s_df_sub, 
    ...
    guides(fill = FALSE, shape = guide_legend(keyheight = 0.8, label.theme = element_text(size = 9)), color = guide_legend(label.theme = element_text(face = "italic", size = 9), keyheight = 0.3, nrow = 2, byrow = TRUE, override.aes = list(linetype = c(0, 0)))) +
    theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), legend.title = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), legend.background = element_rect(fill = "transparent", colour = NA)) +
  annotate("text", x = -0.42, y = -0.38, label = deparse(s_sub_symb_pval), parse = TRUE, size = 3) +
  ggsave("Figures/Final_Figures/Figure4_SSID_PhysPCA.pdf", width = 8, height = 6)
...
corrgram(sid_sub_corr_df, order = TRUE, lower.panel = panel.fill.R2, upper.panel = panel.pts.col, text.panel = panel.txt.brdr,
         col.regions = colorRampPalette(c("#ffffcc", "#c7e9b4", "#225ea8")), 
         color = sid_sub_corr_df$colour, pch = as.numeric(as.character(sid_sub_corr_df$shape)),
         labels = c("symbiont \ndensity", "host \nprotein", "chlorophyll a", "colour \nintensity", "calcification \nrate", "host \ncarbohydrate", "host \nlipid")) 

corrgram(sid_sub_corr_C_df, order = TRUE, lower.panel = panel.fill.R2, upper.panel = panel.pts.col, text.panel = panel.txt.brdr,
         col.regions = colorRampPalette(c("#ffffcc", "#c7e9b4", "#225ea8")), 
         color = sid_sub_corr_C_df$colour, pch = as.numeric(as.character(sid_sub_corr_C_df$shape)),
         labels = c("symbiont \ndensity", "host \nprotein", "chlorophyll a", "colour \nintensity", "calcification \nrate", "host \ncarbohydrate", "host \nlipid")) 

corrgram(sid_sub_corr_D_df, order = TRUE, lower.panel = panel.fill.R2, upper.panel = panel.pts.col, text.panel = panel.txt.brdr,
         col.regions = colorRampPalette(c("#ffffcc", "#c7e9b4", "#225ea8")), 
