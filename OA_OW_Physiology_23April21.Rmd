...
## Physiological paramters (working) {.tabset}
...
plast_aic$treat <- rownames(plast_aic) # add column for rownames
plast_aic$species <- c(rep("SSID", 6), rep("PSTR", 6), rep("PAST", 6)) # add column for species ID
plast_aic <- plast_aic[c(6, 1:5, 7)] # reorder columns so treatment is first
