...
#### GLM and bootstrapping SSID plasticity distance
## Fit the GLM
...
newdata <- data.frame(reef = sid_dist$reef, fpco2 = sid_dist$fpco2, ftemp = sid_dist$ftemp) # creates df of predictor variables
boot <- replicate(bootnum, bootFUN(model = ssid_dist_glm, newdata = newdata)) # run the bootFUN the number (bootnum) of times specified and save as matrix

## Calculate the mean, 95% lowerCI, and 95% upperCI from the boot matrix and add it to dataframe (but as a new name)
sid_dist_boot <- cbind(sid_dist, as.data.frame(t(apply(boot, 1, function(x) c(mean(x), quantile(x, c(0.025, 0.975)))))))
colnames(sid_dist_boot)[16:18] <- c("mean", "lowerci", "upperci") # rename mean/CI columns
