...
p_pco2_pval <-substitute(italic(P[temp])==p, list(p = format(p_pval[2,1], digits = 3)))
p_temp_pval <-substitute(italic(P[pCO[2]])==p, list(p = format(p_pval[3,1], digits = 2)))
...
summary(ssid_dist_glm) # summary output of the GLM
Anova(ssid_dist_glm, type = "III", test.statistic = "F") # comparison of the 'main effects'
...
summary(pstr_dist_glm) # summary output of the GLM
Anova(pstr_dist_glm, type = "III", test.statistic = "F") # comparison of the 'main effects'
...
## Best-fit GLM with Gamma log link
past_dist_glm <- glm(dist ~ reef + fpco2 + ftemp, family = Gamma(link = "log"), data = por_dist)
summary(past_dist_glm) # summary output of the GLM
Anova(past_dist_glm, type = "III", test.statistic = "F") # comparison of the 'main effects'
...
#### *Methods:*
Using the subset of *S. siderea* fragments with known Symbiodinacaea community composition identified from transcriptomic analyses, we further assessed the differences in coral holobiont physiological responses by dominant symbiont genera. Relative abundance of Symbiodinacaea genus within each fragment aas assigned as either *Cladocopium spp.* dominated (> 50% reads mapping to *Cladocopium*) or *Durusdinium spp.* dominated (> 50% reads mapping to *Durusdinium*). A PCA was performed with all coral holobiont physiological parameters with the addition of parameter representing percent of reads mapping to *Cladocopium spp.* such that higher percentages represented *Cladocopium* dominated fragments and lower percentages represented *Durusdinium* dominated fragments. The phyenotypic plasticity of the subset of *S. siderea* fragments was also assessed using the same methods described above. The effects of dominant Symbiodinacaea genus and treatment (*p*CO~2~ and temperature) were assessed using a generalized linear models (function *glm*) with a Gamma distribution and log-link and main effects were assessed with an ANOVA (package *car*; version `r packageVersion("car")`) with a type III error.
...
s_sub_symb_pca <- autoplot(s_sub_pca, data = s_df_sub, 
         ...
         loadings.label.vjust = 0.5,
         loadings.label.repel = TRUE) +
    scale_shape_manual(values = c(21, 23, 24, 22), labels = c("pre industrial", "current", "end-of-century", "extreme")) +
    scale_colour_manual("domSymb", labels = c("Cladocopium spp.", "Durusdinium spp."), values = c(wes_palette("Royal1", 3:5, type = "continuous"))) +
    #scale_fill_manual("domSymb", labels = c("Cladocopium spp.", "Durusdinium spp."), values = c(wes_palette("Royal1", 3:5, type = "continuous"))) +
    guides(fill = FALSE, shape = FALSE, color = guide_legend(label.theme = element_text(face = "italic", size = 9), keyheight = 0.3, nrow = 2, byrow = TRUE, override.aes = list(linetype = c(0, 0)))) +
    theme_pubr() +
    theme(legend.title = element_blank(), legend.position = c(0.14, 0.95)) +
    #theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), legend.title = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), legend.background = element_rect(fill = "transparent", colour = NA)) +
  annotate("text", x = -0.42, y = -0.38, label = deparse(s_sub_symb_pval), parse = TRUE, size = 3)
...
s_sub_temp_pca <- autoplot(s_sub_pca, data = s_df_sub, 
  ...
  scale_colour_manual("temperature", labels = c("28C", "31C"), values = c("#0072B2", "#D55E00"))+
  scale_fill_manual("temperature", labels = c("28C", "31C"), values = c("#0072B2", "#D55E00"))+
  guides(color = guide_legend(keyheight = 0.3, nrow = 2, byrow = TRUE, override.aes = list(linetype = c(0, 0))), shape = guide_legend(keyheight = 0.3, nrow = 4, byrow = TRUE)) +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(),strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank(), legend.background = element_rect(fill = "transparent", colour = NA), legend.position = "bottom") +
  annotate("text", x = -0.30, y = -0.38, label = deparse(s_sub_temp_pval), parse = TRUE, size = 3)
...
s_sub_pco2_pca <- autoplot(s_sub_pca, data = s_df_sub, 
    ...
    scale_fill_manual("pCO2", labels = c("pre industrial", "current", "end-of-century", "extreme"), values = c(NA, NA, NA, NA))+
    scale_colour_manual("pCO2", labels = c("pre industrial", "current", "end-of-century", "extreme"), values = c("#b2abd2", "#5e3c99", "#fdb863", "#e66101"))+
    guides(fill = FALSE, color = guide_legend(order = 1, keyheight = 0.3, nrow = 4, byrow = TRUE, override.aes = list(linetype = c(0, 0, 0, 0))), shape = guide_legend(keyheight = 0.3, nrow = 2, byrow = TRUE)) +
    theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), legend.title = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), legend.background = element_rect(fill = "transparent", colour = NA), legend.position = "bottom") +
  annotate("text", x = -0.30, y = -0.38, label = deparse(s_sub_pco2_pval), parse = TRUE, size = 3)
...
s_sub_reef_pca <- autoplot(s_sub_pca, data = s_df_sub, 
         ...
         loadings.label.vjust = 0.5,
         loadings.label.repel = TRUE) +
  scale_shape_manual("reef environment", labels = c("offshore", "inshore"), values = c(1,2)) +
  scale_colour_manual("reef environment", labels = c("offshore", "inshore"), values = c("#00a08b", "#e9aa2b")) +
  guides(fill = FALSE, color = guide_legend(keyheight = 0.3, nrow = 2, override.aes = list(linetype = c(0, 0))), shape = guide_legend(keyheight = 0.3, nrow = 2, override.aes = list(linetype = c(0, 0)))) +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(),strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), legend.title = element_blank(), legend.background = element_rect(fill = "transparent", colour = NA), legend.position = "bottom") +
  annotate("text", x = -0.345, y = -0.38, label = deparse(s_sub_reef_pval), parse = TRUE, size = 3)
...
Anova(ssid_sub_dist_glm, type = "III", test.statistic = "F") # comparison of the 'main effects'
...
ssid_sub_plast_plot <- ggplot(sid_sub_dist_sum, aes(x = domSymb, y = mean, shape = fpco2, fill = fpco2)) +
  theme_pubr() +
  theme(legend.title = element_blank(), legend.position = c(0.08, 0.87), axis.text.x = element_text(face = "italic"), axis.ticks.x = element_blank()) +
...
```{r save SSID subset PCA and plasticity, fig.width = 13, fig.height = 6}

ggarrange(s_sub_symb_pca, ssid_sub_plast_plot, ncol = 2, labels = "AUTO") +
  ggsave("Figures/Final_Figures/Figure4_SSIDPhys.pdf", width = 13, height = 6)

```

**Figure 4.** *Text will go here*
...
```{r Figure S2 SSID subset PCA all plots, fig.height = 4, fig.width = 12}

ggarrange(s_sub_pco2_pca, s_sub_temp_pca, s_sub_reef_pca, nrow = 1, labels = "AUTO")

```

**Figure S2.** *Text will go here*
