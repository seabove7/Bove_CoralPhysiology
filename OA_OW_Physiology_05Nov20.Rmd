...
```{r SSID PCA plot, fig.height=3, fig.width=8}
...
SSID_plot <- plot_grid(s_temp_pca, s_pco2_pca, s_reef_pca, ncol = 3, scale = 0.8)
SSID_plot
...
```{r PSTR PCA plot, fig.height=3, fig.width=8}
...
PSTR_plot <- plot_grid(p_temp_pca, p_pco2_pca, p_reef_pca, ncol = 3, scale = 0.8)
PSTR_plot
...
```{r PAST PCA plot, fig.height=4, fig.width=8}
...
PAST_plot <- plot_grid(a_tema_pca, a_pco2_pca, a_reef_pca, ncol = 3, scale = 0.8)
PAST_plot
...
plot_grid(SSID_plot, PSTR_plot, PAST_plot, ncol = 1, labels = c("A", "B", "C"), rel_heights = c(0.82, 0.75, 1)) +
  ggsave("Figure2_PhysPCA.pdf", width = 11, height = 12)
...
s.dis <- s_pca$x[,1:2] # grab PC1 and PC2 distances from prcomp() object

sid.dist <- cbind(s_df[,c(1,7,10,11,12,9,25)], s.dis) # combine the datasets
...
a.dis <- a_pca$x[-22,1:2] # grab PC1 and PC2 distances from prcomp() object

por.dist <- cbind(a_df[-22,c(1,7,10,11,12,9,21)], a.dis)
