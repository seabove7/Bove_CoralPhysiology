...
param_list <- levels(df2_T90$param)[-4]
...
for (p in 1:length(param_list)) {
  ...
  if (df[1,15] == "count") {
    glm_full <- lme4::glmer.nb(value ~ species * fpco2 * ftemp * reef + offset(log(offset)) + (1 | colony), data = df)
...
df_subset <- as.data.frame(df_90[,c(29,8,14:18,21:24,25,27:29)]) # subset the df for desired columns - with dominant symbionts data
...
sid_corr_df_sub <- rcorr(as.matrix(sid_df_sub[, c(4,9,10,3,5,6,8,13)])) # from these correlations, going to stick with red channel for corr/PCA input
...
sid_corr_df_C <- rcorr(as.matrix(sid_corr_C[, c(4,9,10,3,5,6,8,13)])) # from these correlations, going to stick with red channel for corr/PCA input
...
sid_corr_df_D <- rcorr(as.matrix(sid_corr_D[, c(4,9,10,3,5,6,8,13)])) # from these correlations, going to stick with red channel for corr/PCA input
...
## Rmarkdown plotting version
par(mfrow=c(1,3))
corrplot.mixed(sid_corr_df_sub$r, upper = "ellipse", lower.col = "darkslategrey", upper.col = wes_palette("Zissou1", 7, type = "continuous"), number.cex = 0.8, p.mat = sid_corr_df_sub$P, sig.level = c(.001, .01, .05), insig = "blank", tl.col = "#0072B2", tl.cex = 1.1, title = expression(italic("S. siderea")), cl.lim = c(0,1), mar=c(0,0,1,0), cl.align.text = "l")

corrplot.mixed(sid_corr_df_C$r, upper = "ellipse", lower.col = "darkslategrey", upper.col = wes_palette("Zissou1", 7, type = "continuous"), number.cex = 0.8, p.mat = sid_corr_df_C$P, sig.level = c(.001, .01, .05), insig = "blank", tl.col = "#0072B2", tl.cex = 1.1, title = expression(italic("S. siderea - Cladocopium")), cl.lim = c(0,1), mar=c(0,0,1,0), cl.align.text = "l")

corrplot.mixed(sid_corr_df_D$r, upper = "ellipse", lower.col = "darkslategrey", upper.col = wes_palette("Zissou1", 7, type = "continuous"), number.cex = 0.8, p.mat = sid_corr_df_D$P, sig.level = c(.001, .01, .05), insig = "blank", tl.col = "#0072B2", tl.cex = 1.1, title = expression(italic("S. siderea - Durusdinium")), cl.lim = c(0,1), mar=c(0,0,1,0), cl.align.text = "l")
...
ggarrange(temp_PCA, pCO2_PCA, nrow = 2) +
  ggsave("Figures/Final_Figures/Figure2_PhysPCA_23March21.pdf", width = 13, height = 8)
...
s_df_sub_l <- gather(s_df_sub, param, value, c(14:17,21:23,27))
...
s_pca_sub_mod <- adonis(value ~ ftemp + fpco2 + domSymb, data = s_df_sub_l, method='eu')
s_pca_sub_mod # view SSID adonis output

# extract pvalues
s_sub_pval <- s_pca_sub_mod[["aov.tab"]][["Pr(>F)"]]

# perform principal component analysis (PCA)
s_sub_pca <- prcomp(s_df_sub[,c(14:17,21:23,27)], center = TRUE, scale= TRUE)
...
# create labels for p values calculated above
s_sub_reef_pval <-substitute(italic(P[reef])==p, list(p = format(s_sub_pval[1], digits = 2)))
s_sub_temp_pval <-substitute(italic(P[temp])==p, list(p = format(s_sub_pval[2], digits = 3)))
s_sub_pco2_pval <-substitute(italic(P[pCO[2]])==p, list(p = format(s_sub_pval[3], digits = 2)))
...
sid_pcWT <- summary(s_pca)$importance[2,1:2]

sid_dist <- cbind(s_df[,c(1,7,10,11,12,9,25)], s_dis) # combine the datasets

# going to also try to weight the PC distance by the overall variation 

sid_dist$PC1_wt <- sid_dist$PC1 * sid_pcWT[1]
sid_dist$PC2_wt <- sid_dist$PC2 * sid_pcWT[2]
...
sid_dist$cont_pc1_wt[sid_dist$treat2 == "447_28"] <- sid_dist$PC1_wt[sid_dist$treat2 == "447_28"]
sid_dist$cont_pc2_wt[sid_dist$treat2 == "447_28"] <- sid_dist$PC2_wt[sid_dist$treat2 == "447_28"]
...
sid_dist2 <- sid_dist %>%  
  ...
  summarise(con2_pc1 = sum(cont_pc1, na.rm = TRUE),
            ...
            con2_pc1_wt = sum(cont_pc1_wt, na.rm = TRUE),
            con2_pc2_wt = sum(cont_pc2_wt, na.rm = TRUE))
...
sid_dist$dist_wt <- sqrt(((sid_dist$PC1_wt - sid_dist$con2_pc1_wt)^2) + ((sid_dist$PC2_wt - sid_dist$con2_pc2_wt)^2)) # formula for calculating distance between control (T90) and treatment values
...
summary(lm(dist ~ fpco2 * ftemp * reef, data = sid_dist))
summary(lm(dist_wt ~ fpco2 * ftemp * reef, data = sid_dist))
...
sid_con_dis_plot <- ggplot(sid_dist_sum2) +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), strip.text.x = element_blank(), panel.background=element_blank(), legend.key=element_blank(), strip.background = element_blank(), legend.position="bottom", panel.border = element_rect(colour = "black", size= 0.8)) +
  #scale_x_continuous(breaks = c(2, 4 ,6 ,8, 10), labels = c("T0", "300", "420", "680", "3300"))+
  ylab("Mean distance from control") +
  xlab("") +
  #ylim(0, 7) +
  #geom_point(aes(x = treat, y = dist, colour = reef), position = dodge, alpha = 0.3, shape = 17, size = 2) +
  geom_point(aes(x = treat_plot, y = dist, colour = reef, group = reef), position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.2), alpha = 0.3, size = 1.8, data = sid_dist) +
  geom_point(position = dodge, aes(x = treat, y = dist, colour = reef), shape = "-", size = 8) +  
  geom_errorbar(position = dodge, aes(x = treat, ymin = dist - ci, ymax = dist + ci, colour = reef), width = 0, size = 1.5) +
...
# just treatment (weighted)
sid_dist_sum2_wt <- summarySE(measurevar = "dist_wt", groupvars = c("fpco2", "ftemp", "reef"), data = sid_dist)
sid_dist_sum2_wt$treat <- paste(sid_dist_sum2_wt$fpco2, sid_dist_sum2_wt$ftemp, sep = "_")

sid_con_dis_wt_plot <- ggplot(sid_dist_sum2_wt) +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), strip.text.x = element_blank(), panel.background=element_blank(), legend.key=element_blank(), strip.background = element_blank(), legend.position="none", panel.border = element_rect(colour = "black", size= 0.8)) +
  #scale_x_continuous(breaks = c(2, 4 ,6 ,8, 10), labels = c("T0", "300", "420", "680", "3300"))+
  ylab("Mean distance from control") +
  xlab("") +
  #ylim(0, 7) +
  #geom_point(aes(x = treat, y = dist, colour = reef), position = dodge, alpha = 0.3, shape = 17, size = 2) +
  geom_point(aes(x = treat_plot, y = dist_wt, colour = reef, group = reef), position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.2), alpha = 0.3, size = 1.8, data = sid_dist) +
  geom_point(position = dodge, aes(x = treat, y = dist_wt, colour = reef), shape = "-", size = 8) +    
  geom_errorbar(position = dodge, aes(x = treat, ymin = dist_wt - se, ymax = dist_wt + se, colour = reef), width = 0, size = 1.5) +
  geom_vline(xintercept = c(1.5, 3.5, 4.5, 6.5)) +
  annotate("text", x = 0.8, y = 7, label = "control") +  
  annotate("text", x = 2, y = 7, label = "pre-industrial") +
  annotate("text", x = 4, y = 7, label = "present-day") +
...
plot_grid(sid_con_dis_wt_plot, sid_con_dis_plot, ncol = 1, rel_heights = c(1,1.3)) +
  ggsave("Figures/Supplemental_Figures/SSID_plasticity_09Mar21.pdf", width = 10, height = 7)
...
dip_pcWT <- summary(p_pca)$importance[2,1:2]

dip_dist <- cbind(p_df[,c(1,7,10,11,12,9,25)], p_dis) # combine the datasets

# going to also try to weight the PC distance by the overall variation 

dip_dist$PC1_wt <- dip_dist$PC1 * dip_pcWT[1]
dip_dist$PC2_wt <- dip_dist$PC2 * dip_pcWT[2]
...
dip_dist$cont_pc1_wt[dip_dist$treat2 == "447_28"] <- dip_dist$PC1_wt[dip_dist$treat2 == "447_28"]
dip_dist$cont_pc2_wt[dip_dist$treat2 == "447_28"] <- dip_dist$PC2_wt[dip_dist$treat2 == "447_28"]
...
dip_dist$dist_wt <- sqrt(((dip_dist$PC1_wt - dip_dist$con2_pc1_wt)^2) + ((dip_dist$PC2_wt - dip_dist$con2_pc2_wt)^2)) # formula for calculating distance between control (T90) and treatment values
...
summary(lm(dist ~ fpco2 * ftemp * reef, data = dip_dist))
summary(lm(dist_wt ~ fpco2 * ftemp * reef, data = dip_dist))
...
dip_con_dis_plot <- ggplot(dip_dist_sum2) +
  ...
  #ylim(0, 7) +
  #geom_point(aes(x = treat, y = dist, colour = reef), position = dodge, alpha = 0.3, shape = 17, size = 2) +
  geom_point(aes(x = treat_plot, y = dist, colour = reef, group = reef), position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.2), alpha = 0.3, size = 1.8, data = dip_dist) +
  geom_point(position = dodge, aes(x = treat, y = dist, colour = reef), shape = "-", size = 8) +  
  geom_errorbar(position = dodge, aes(x = treat, ymin = dist - ci, ymax = dist + ci, colour = reef), width = 0, size = 1.5) +
  geom_vline(xintercept = c(1.5, 3.5, 4.5, 6.5)) +
  annotate("text", x = 0.8, y = 5, label = "control") +  
  annotate("text", x = 2, y = 5, label = "pre-industrial") +
  annotate("text", x = 4, y = 5, label = "present-day") +
  annotate("text", x = 5.1, y = 5, label = "end-of-century") +
  annotate("text", x = 6.9, y = 5, label = "extreme") +
  scale_colour_manual("reef environment", labels = c("offshore", "inshore"), values = c("#00a08b", "#e9aa2b")) +
  ggtitle("Unweighted")


# just treatment (weighted)
dip_dist_sum2_wt <- summarySE(measurevar = "dist_wt", groupvars = c("fpco2", "ftemp", "reef"), data = dip_dist)
dip_dist_sum2_wt$treat <- paste(dip_dist_sum2_wt$fpco2, dip_dist_sum2_wt$ftemp, sep = "_")

dip_con_dis_wt_plot <- ggplot(dip_dist_sum2_wt) +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), strip.text.x = element_blank(), panel.background=element_blank(), legend.key=element_blank(), strip.background = element_blank(), legend.position="none", panel.border = element_rect(colour = "black", size= 0.8)) +
  #scale_x_continuous(breaks = c(2, 4 ,6 ,8, 10), labels = c("T0", "300", "420", "680", "3300"))+
  ylab("Mean distance from control") +
  ...
  #ylim(0, 7) +
  #geom_point(aes(x = treat, y = dist, colour = reef), position = dodge, alpha = 0.3, shape = 17, size = 2) +
  geom_point(aes(x = treat_plot, y = dist_wt, colour = reef, group = reef), position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.2), alpha = 0.3, size = 1.8, data = dip_dist) +
  geom_point(position = dodge, aes(x = treat, y = dist_wt, colour = reef), shape = "-", size = 8) +    
  geom_errorbar(position = dodge, aes(x = treat, ymin = dist_wt - se, ymax = dist_wt + se, colour = reef), width = 0, size = 1.5) +
  geom_vline(xintercept = c(1.5, 3.5, 4.5, 6.5)) +
  annotate("text", x = 0.8, y = 5, label = "control") +  
...
## Save combined plot
plot_grid(dip_con_dis_wt_plot, dip_con_dis_plot, ncol = 1, rel_heights = c(1,1.3)) +
  ggsave("Figures/Supplemental_Figures/PSTR_plasticity_09Mar21.pdf", width = 10, height = 7)
...
por_pcWT <- summary(a_pca)$importance[2,1:2]

por_dist <- cbind(a_df[,c(1,7,10,11,12,9,25)], a_dis) # combine the datasets

# going to also try to weight the PC distance by the overall variation 

por_dist$PC1_wt <- por_dist$PC1 * por_pcWT[1]
por_dist$PC2_wt <- por_dist$PC2 * por_pcWT[2]
...
por_dist$cont_pc1_wt[por_dist$treat2 == "447_28"] <- por_dist$PC1_wt[por_dist$treat2 == "447_28"]
por_dist$cont_pc2_wt[por_dist$treat2 == "447_28"] <- por_dist$PC2_wt[por_dist$treat2 == "447_28"]
...
por_dist$dist_wt <- sqrt(((por_dist$PC1_wt - por_dist$con2_pc1_wt)^2) + ((por_dist$PC2_wt - por_dist$con2_pc2_wt)^2)) # formula for calculating distance between control (T90) and treatment values
...
summary(lm(dist ~ fpco2 * ftemp * reef, data = por_dist))
summary(lm(dist_wt ~ fpco2 * ftemp * reef, data = por_dist))
...
por_con_dis_plot <- ggplot(por_dist_sum2) +
  ...
  #ylim(0, 7) +
  #geom_point(aes(x = treat, y = dist, colour = reef), position = dodge, alpha = 0.3, shape = 17, size = 2) +
  geom_point(aes(x = treat_plot, y = dist, colour = reef, group = reef), position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.2), alpha = 0.3, size = 1.8, data = por_dist) +
  geom_point(position = dodge, aes(x = treat, y = dist, colour = reef), shape = "-", size = 8) +  
  geom_errorbar(position = dodge, aes(x = treat, ymin = dist - ci, ymax = dist + ci, colour = reef), width = 0, size = 1.5) +
  geom_vline(xintercept = c(1.5, 3.5, 4.5, 6.5)) +
  annotate("text", x = 0.8, y = 4, label = "control") +  
  annotate("text", x = 2, y = 4, label = "pre-industrial") +
  annotate("text", x = 4, y = 4, label = "present-day") +
  annotate("text", x = 5.1, y = 4, label = "end-of-century") +
  annotate("text", x = 6.9, y = 4, label = "extreme") +
  scale_colour_manual("reef environment", labels = c("offshore", "inshore"), values = c("#00a08b", "#e9aa2b")) +
  ggtitle("Unweighted")


# just treatment (weighted)
por_dist_sum2_wt <- summarySE(measurevar = "dist_wt", groupvars = c("fpco2", "ftemp", "reef"), data = por_dist)
por_dist_sum2_wt$treat <- paste(por_dist_sum2_wt$fpco2, por_dist_sum2_wt$ftemp, sep = "_")

por_con_dis_wt_plot <- ggplot(por_dist_sum2_wt) +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), strip.text.x = element_blank(), panel.background=element_blank(), legend.key=element_blank(), strip.background = element_blank(), legend.position="none", panel.border = element_rect(colour = "black", size= 0.8)) +
  #scale_x_continuous(breaks = c(2, 4 ,6 ,8, 10), labels = c("T0", "300", "420", "680", "3300"))+
  ylab("Mean distance from control") +
  ...
  #ylim(0, 7) +
  #geom_point(aes(x = treat, y = dist, colour = reef), position = dodge, alpha = 0.3, shape = 17, size = 2) +
  geom_point(aes(x = treat_plot, y = dist_wt, colour = reef, group = reef), position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.2), alpha = 0.3, size = 1.8, data = por_dist) +
  geom_point(position = dodge, aes(x = treat, y = dist_wt, colour = reef), shape = "-", size = 8) +    
  geom_errorbar(position = dodge, aes(x = treat, ymin = dist_wt - se, ymax = dist_wt + se, colour = reef), width = 0, size = 1.5) +
  geom_vline(xintercept = c(1.5, 3.5, 4.5, 6.5)) +
  annotate("text", x = 0.8, y = 4, label = "control") +  
...
plot_grid(por_con_dis_wt_plot, por_con_dis_plot, ncol = 1, rel_heights = c(1,1.3)) +
  ggsave("Figures/Supplemental_Figures/PAST_plasticity_09Mar21.pdf", width = 10, height = 7)
