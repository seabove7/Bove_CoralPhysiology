...
## This is a bootstrap function that will sample WITH replacement (explained step by step within the function) -- for GLMER object
...
# check for best-fit model
pstr_plast_aic <- compare_performance(pstr_dist_mod, pstr_dist_mod2, pstr_dist_mod3, pstr_dist_mod4, pstr_dist_mod5, pstr_dist_mod6, pstr_dist_mod7, rank = TRUE)
plot(pstr_plast_aic)
...
## Plasticity plot
past_plast_plot <- ggplot(por_dist_sum, aes(x = reef, y = mean, color = treat2, fill = treat2, shape = treat2)) +
  theme_pubr() +
  theme(legend.title = element_blank(), axis.ticks.x = element_blank()) +
  guides(shape = guide_legend(ncol = 1), fill = guide_legend(override.aes = list(linetype = 0)), color = guide_legend(override.aes = list(linetype = 0))) +
...
dist_mod_out$species <- c(rep("SSID", 9), rep("PSTR", 5), rep("PAST", 5)) # add column for species ID
...
table_s3 <- kable(dist_mod_out[,-6], booktabs = TRUE, row.names = FALSE) %>%
  ...
  pack_rows("Pseudodiploria strigosa", 10, 14, italic = TRUE) %>%
  pack_rows("Porites astreoides", 15, 19, italic = TRUE)
table_s3
