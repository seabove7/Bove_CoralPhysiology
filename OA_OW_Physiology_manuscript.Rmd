...
df_withT_phys <- df_withT_phys %>% 
  mutate(param = factor(param, levels = c("carb", "lipid", "pro", "host", "chla", "den", "red", "sum"), labels = c(expression(paste("Carbohydrate (mg cm"^-2,")")), expression(paste("Lipid (mg cm"^-2,")")), expression(paste("Protein (mg cm"^-2,")")), expression(paste("Total Host (mg cm"^-2,")")), expression(paste("Chlorophyll a ("*mu,"g cm"^-2,")")), expression(paste("Density (10"^6,"cells cm"^-2,")")), expression(paste("Red"," channel")), expression(paste("Sum"," channel")))),
         species = factor(species, levels = c("S", "P", "A", "T"), labels = c(expression(italic("Siderastrea siderea")), expression(italic("Pseudodiploria strigosa")), expression(italic("Porites astreoides")), expression(italic("Undaria tenuifolia")))))
...
test <- df[,-c(13:16, 37:42)]
test <- completeFun(test, "sum") # run function to remove any missing values
...
df_noT_colour <- gather(test, tp, colour, c(13,14,17,18,21,22,25,26,30))



# reorder the parameters and assign labels for plotting
df_noT_colour <- df_noT_colour %>% 
  mutate(colour = (colour * -1),
         tp = factor(tp, levels = c("red_bw1", "red_bw2", "red_bw3", "red_bw4", "red", "sum_bw1", "sum_bw2", "sum_bw3", "sum_bw4", "sum"), labels = c("pre-acclimation", "t0", "t30", "t60", "t90", "Pre-acclimation", "T0", "T30", "T60", "T90")),
         species = factor(species, levels = c("S", "P", "A"), labels = c(expression(italic("Siderastrea siderea")), expression(italic("Pseudodiploria strigosa")), expression(italic("Porites astreoides")))))

  

ggplot(df_noT_colour %>% filter(tp == c("pre-acclimation", "t0", "t30", "t60", "t90")), aes(x = tp, y = colour, colour = ftemp, group = ftemp)) +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), legend.position = c(0.96, 0.982), legend.background = element_rect(fill = NA)) + 
  geom_point(alpha = 0.3, position = position_dodge(width = 0.2)) +
  stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = 0, position = position_dodge(width = 0.2)) +
  stat_summary(fun = "mean", size = 0.3, position = position_dodge(width = 0.2), geom = "line", aes(group = treat2)) +
  #stat_summary(fun = "mean", size = 0.3, position = position_dodge(width = 0.2), geom = "point", aes(group = treat2), size = 7) +  
  #stat_summary(fun.y = mean, fun.ymin = function(x) mean(x) - sd(x),  fun.ymax = function(x) mean(x) + sd(x),  geom = "pointrange", position = position_dodge(width = 0.2)) +
  #stat_summary(fun.y = mean, geom = "line", aes(group = treat2), position = position_dodge(width = 0.2)) +
  facet_grid(species ~ fpco2, scales = "free_y", labeller = label_parsed) +
  scale_colour_manual("", labels = c("28 °C", "31 °C"), values = c("#4393c3", "#b2182b")) +  
  labs(x = "time point", y = "colour intensity (sum of RGB channels)")
...
ggplot(df_noT_colour %>% filter(tp == c("Pre-acclimation", "T0", "T30", "T60", "T90")), aes(x = tp, y = colour, colour = ftemp, group = ftemp)) +
  ...
  #stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = 0, position = position_dodge(width = 0.2)) +
  #stat_summary(fun = "mean", size = 0.3, position = position_dodge(width = 0.2), geom = "line") +
  #stat_summary(fun = "mean", size = 0.3, position = position_dodge(width = 0.2), geom = "point") +  
  facet_grid(species ~ fpco2, scales = "free_y", labeller = label_parsed) +
  scale_colour_manual("", labels = c("28 °C", "31 °C"), values = c("#4393c3", "#b2182b")) +  
  labs(x = "time point", y = "colour intensity (sum of RGB channels)")
