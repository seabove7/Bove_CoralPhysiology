...
## Plasticity plot
pstr_plast_plot <- ggplot(pstr_boot, aes(x = reef, y = estimate, color = treat2, fill = treat2, shape = treat2)) +
  theme_pubr() +
  theme(legend.title = element_blank(), axis.ticks.x = element_blank()) +
  guides(shape = guide_legend(ncol = 1), fill = guide_legend(override.aes = list(linetype = 0)), color = guide_legend(override.aes = list(linetype = 0))) +
  geom_point(aes(x = reef, y = dist), size = 2, alpha = 0.4, position = position_jitterdodge(jitter.width = 0.3, dodge.width = 0.55)) +
...
## Model selection (via AIC)
past_dist_mod <- glmer(dist ~ reef * fpco2 * ftemp + (1 | colony), family = Gamma(link = "log"), data = por_dist) # best fit
...
past_dist_mod6 <- glmer(dist ~ fpco2 + ftemp + (1 | colony), family = Gamma(link = "log"), data = por_dist) 
past_dist_mod1b <- glmer(dist ~ reef * fpco2 * ftemp + (1 | colony) + (1 | tank), family = Gamma(link = "log"), data = por_dist) 

# check for best-fit model
past_plast_aic <- compare_performance(past_dist_mod, past_dist_mod2, past_dist_mod3, past_dist_mod4, past_dist_mod5, past_dist_mod6, past_dist_mod1b, rank = TRUE)
plot(past_plast_aic)


## Best-fit GLMM with Gamma log link
past_dist_glm <- glmer(dist ~ reef * fpco2 * ftemp + (1 | colony) + (1 | tank), family = Gamma(link = "log"), data = por_dist)
