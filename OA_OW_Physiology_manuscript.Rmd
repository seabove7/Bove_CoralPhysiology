...
# reef
s_ptem_pca <- autoplot(s_pca, data = s_df, 
         colour = "reef",
         fill = "reef",
         frame = TRUE, 
         frame.type = "t", # displaying ellipses with multivariate t-distributions for small n producing heavier tails
...
### PCA all species at control


```{r all PCA}

# set up the dataframe
all_df_cntrl <- filter(all_df, pco2 == 447)
all_cntrl_pca <- all_df_cntrl[,c(14:17,21:23)]

# run the adonis
#all_pca_mod <- adonis2(all_pca_df ~ reef * ftemp * fpco2 * species, data = all_df, method = 'eu', permutations = bootnum) # below is the model with non sig interactions removed:
all_pca_cntrl_mod <- adonis2(all_cntrl_pca ~ species, data = all_df_cntrl, method = 'eu', permutations = bootnum)
all_pca_cntrl_mod # view all adonis output

# extract pvalues
all_pval <- all_pca_cntrl_mod["Pr(>F)"]

# perform principal component analysis (PCA)
all_cntrl_pca <- prcomp(all_cntrl_pca, center = TRUE, scale= TRUE)

```

```{r all PCA plot, fig.height=6, fig.width=6}

# PCA plot of all species at control treatment only
autoplot(all_cntrl_pca, data = all_df_cntrl, 
                       colour = "species",
                       #shape = "species",
                       #shape = "fpco2",
                       #fill = "species",               
...
sid_dist <- PCAplast(pca = s_pca,
                     data = s_df[,c(1,7,8, 10,11,12,27,2)],
                     control_col = "treat2",
                     control_lvl = "420_28")
...
colnames(ssid_boot)[24:26] <- c("estimate", "lowerci", "upperci") # rename mean/CI columns
...
dip_dist <- PCAplast(pca = p_pca,
                     data = p_df[,c(1,7,8, 10,11,12,27,2)],
                     control_col = "treat2",
                     control_lvl = "420_28")
...
#pstr_dist_mod6b <- glmer(dist ~ fpco2 + ftemp + (1 | colony) + (1 | tank), family = Gamma(link = "log"), data = dip_dist) # singular
...
por_dist <- PCAplast(pca = a_pca,
                     data = a_df[,c(1,7,8, 10,11,12,27, 2)],
                     control_col = "treat2",
                     control_lvl = "420_28")
