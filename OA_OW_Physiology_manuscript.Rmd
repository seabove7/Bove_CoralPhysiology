...
bootFUN <- function(model, newdata) {
  ...
  predict(update, newdata, type = "response", re.form = NA) # predicts response variable with model and updated data 
}
...
ssid_plast_plot <- ggplot(ssid_boot, aes(x = reef, y = estimate, color = treat2, fill = treat2, shape = treat2)) +
  ...
  geom_point(data = ssid_boot, aes(x = reef, y = dist), size = 2, alpha = 0.7, position = position_dodge(width = 0.55)) +
  geom_linerange(aes(ymin = lowerci, ymax =upperci), alpha = 0.8, size = 1, position = position_dodge(width = 0.55)) +
...
dip_dist_sum <- DistSum_noRZ(data = dip_dist)
...
pstr_dist_mod7 <- glmer(dist ~ fpco2 * ftemp + (1 | colony), family = Gamma(link = "log"), data = dip_dist)

# check for best-fit model
pstr_plast_aic <- compare_performance(pstr_dist_mod, pstr_dist_mod2, pstr_dist_mod3, pstr_dist_mod6, pstr_dist_mod7, rank = TRUE)
plot(pstr_plast_aic)
...
pstr_plast_plot <- ggplot(pstr_boot, aes(x = species, y = estimate, color = treat2, fill = treat2, shape = treat2)) +
  ...
  theme(legend.title = element_blank(), axis.ticks.x = element_blank()) +
  guides(shape = guide_legend(ncol = 1), fill = guide_legend(override.aes = list(linetype = 0)), color = guide_legend(override.aes = list(linetype = 0))) +
  geom_point(data = pstr_boot, aes(x = species, y = dist), size = 2, alpha = 0.7, position = position_dodge(width = 0.55)) +
  geom_linerange(aes(ymin = lowerci, ymax =upperci), alpha = 0.8, size = 1, position = position_dodge(width = 0.55)) +
  geom_point(size = 3, stroke = 1, position = position_dodge(width = 0.55)) +
  scale_shape_manual(values = c(21, 21, 23, 24, 24, 22, 22)) +
...
past_plast_plot <- ggplot(past_boot, aes(x = species, y = estimate, color = treat2, fill = treat2, shape = treat2)) +
  ...
  theme(legend.title = element_blank(), axis.ticks.x = element_blank()) +
  guides(shape = guide_legend(ncol = 1), fill = guide_legend(override.aes = list(linetype = 0)), color = guide_legend(override.aes = list(linetype = 0))) +
  geom_point(data = past_boot, aes(x = species, y = dist), size = 2, alpha = 0.7, position = position_dodge(width = 0.55)) +
  geom_linerange(aes(ymin = lowerci, ymax =upperci), alpha = 0.8, size = 1, position = position_dodge(width = 0.55)) +
  geom_point(size = 3, stroke = 1, position = position_dodge(width = 0.55)) +
  scale_shape_manual(values = c(21, 21, 23, 24, 24, 22, 22)) +
  scale_color_manual(values = c("#b2abd2", "#b2abd2", "#5e3c99", "#fdb863", "#fdb863", "#e66101", "#e66101")) + 
...
ggarrange(ssid_plast_plot, pstr_plast_plot, past_plast_plot, common.legend = TRUE, legend = "right", ncol = 3, widths = c(1, 0.6, 1)) +
...
**Figure 3.** Assessment of physiological plasticity of (**A**) *S. siderea*, (**B**) *P. strigosa*, and (**C**) *P. astreoides* in experimental treatments and by natal reef environment. Higher values represent greater plasticity in coral holobiont samples. *p*CO~2~ treatment is depicted by color and shape (300 $\mu$atm light purple, circle; 420 $\mu$atm dark purple, diamond; 680 $\mu$atm light orange, triangle; 3290 $\mu$atm dark orange, square) and temperature is represented as either closed (28$^\circ$C) or open (31$^\circ$C) symbols.
