...
test <- all_df %>% filter(treat2 == "447_28")
...
all_pco2_pval <-substitute(italic(P[reef])==p, list(p = format(all_pval[1,1], digits = 2)))
all_temp_pval <-substitute(italic(P[temp])==p, list(p = format(all_pval[2,1], digits = 3)))
all_reef_pval <-substitute(italic(P[pCO[2]])==p, list(p = format(all_pval[3,1], digits = 2)))
...
species_pca_plot <- autoplot(all_pca, data = all_df, 
  ...
  theme(legend.title = element_blank(), legend.background = element_rect(fill = "transparent", colour = NA), legend.position = c(0.12, 0.9), legend.text = element_text(face = "italic")) +
...
all_temp_pca <- autoplot(all_pca, data = all_df, 
  ...
  #annotate("text", x = -0.30, y = -0.38, label = deparse(all_temp_pval), parse = TRUE, size = 3) +
...
all_pco2_pca <- autoplot(all_pca, data = all_df, 
  ...
  #annotate("text", x = -0.30, y = -0.38, label = deparse(all_pco2_pval), parse = TRUE, size = 3) +
...
**Figure S2.** Mean ($\pm$SE) physiological parameter (each row) measured for *S. siderea* (**A**), *P. strigosa* (**B**), *P. astreoides* (**C**), and *U. tenuifolia* (**D**) at the completion of the 93-day experimental period. 
...
**Figure S3.** Coral colour changes over the experimental period. Representative images of fragments of *P. astreoides* (**A**), *S. siderea* (**B**), and *P. strigosa* (**C**) from the same colonies demonstrating change in coral colour over time in either control (400 μatm; 28 °C) or warming (400 μatm; 31 °C) treatments from the start of the experiment (T0) to the end (T90).
